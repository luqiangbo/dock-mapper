...
pub(super) fn active_screen(app: &AppHandle) -> Result<ActiveScreen, String> {
    ...
    Ok(ActiveScreen {
        id: (position.x as u32).wrapping_mul(31)
        ...
        physical_origin_x: position.x,
        physical_origin_y: position.y,
...
fn is_blank_bgra(width: u32, height: u32, pixels: &[u8]) -> bool {
    if width < 2 || height < 2 {
        return true;
    }
    let first = &pixels[..4];
    ...
    for pixel in pixels
        .chunks_exact(4)
        .step_by(((width as usize * height as usize) / 600).max(1))
    {
...
fn capture_gdi(rect: &ActiveScreen) -> Result<(Arc<[u8]>, u32, u32), String> {
    unsafe {
        ...
        let info = BITMAPINFO {
            bmiHeader: BITMAPINFOHEADER {
                ...
                biWidth: rect.physical_width as i32,
                biHeight: -(rect.physical_height as i32),
                biPlanes: 1,
                biBitCount: 32,
        ...
        let bitmap = CreateDIBSection(Some(memory), &info, DIB_RGB_COLORS, &mut pixels, None, 0)
            .map_err(|error| error.to_string())?;
        if bitmap.0.is_null() || pixels.is_null() {
        ...
        let result = BitBlt(
            ...
            0,
            0,
            rect.physical_width as i32,
            rect.physical_height as i32,
            Some(desktop),
            rect.physical_origin_x,
            rect.physical_origin_y,
            SRCCOPY | CAPTUREBLT,
        )
        .map_err(|error| error.to_string())
        .and_then(|_| {
            let length = rect.physical_width as usize * rect.physical_height as usize * 4;
            let bytes = slice::from_raw_parts(pixels.cast::<u8>(), length);
            if is_blank_bgra(rect.physical_width, rect.physical_height, bytes) {
                return Err("Screen capture returned a blank image".into());
            }
            Ok((
                bmp_from_bgra(rect.physical_width, rect.physical_height, bytes)?,
                rect.physical_width,
                rect.physical_height,
            ))
        });
...
pub(super) fn capture_active_screen(app: &AppHandle) -> Result<CaptureData, String> {
    ...
    let rect = crate::dxgi_capture::MonitorRect {
        x: screen.physical_origin_x,
        y: screen.physical_origin_y,
    ...
    let ((bmp, image_width, image_height), backend) = capture_with_gdi_fallback(dxgi, |error| {
        tracing::warn!(target: "dock_mapper::capture", %error, "falling back to native GDI capture");
        capture_gdi(&screen)
    })?;
    let scale_factor = image_width as f64 / screen.bounds.width.max(1.0);
    let window_candidates = window_candidates::candidates_for_monitor(
        screen.physical_origin_x,
        screen.physical_origin_y,
        image_width,
        image_height,
        scale_factor,
    );
    Ok(CaptureData {
        ...
        scale_factor,
        screen_id: screen.id,
        physical_origin_x: screen.physical_origin_x,
        physical_origin_y: screen.physical_origin_y,
