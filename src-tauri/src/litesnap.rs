...
const DEFAULT_SHORTCUT_WINDOWS: &str = "Control+1";
...
pub fn create_state() -> AppState {
    AppState {
        // DockMapper owns persistence and startup settings. LiteSnap's local
...
pub fn initialize(app: &AppHandle) -> Result<(), String> {
    ...
    // decodes one Base64 payload and shows a rendered overlay.
    prewarm_overlay(app);
    #[cfg(target_os = "windows")]
    {
    ...
    app.global_shortcut()
        .on_shortcut("Control+1", |app, _, event| {
            if event.state() == ShortcutState::Pressed {
                handle_start_capture(app);
            }
        })
        .map_err(|error| format!("注册 Ctrl+1 截图快捷键失败：{error}"))?;
    app.global_shortcut()
...
fn capture_active_screen(app: &AppHandle) -> Result<CaptureData, String> {
    ...
    let (bmp, image_width, image_height, bounds, physical_origin_x, physical_origin_y) = {
        ...
        let (bmp, image_width, image_height) = match dxgi {
            ...
            Err(error) => {
                ...
                eprintln!("{error}; falling back to compatibility capture");
                let image = screen
    ...
    #[cfg(not(target_os = "windows"))]
    let (bmp, image_width, image_height) = (bmp_from_rgba(&image)?, image.width(), image.height());
...
fn capture_region_image(app: &AppHandle, rect: Rect) -> Result<RgbaImage, String> {
    ...
    let image = {
        ...
        let requested_width = physical_right
            .saturating_sub(physical_x.max(0) as u32)
            .max(1);
        let requested_height = physical_bottom
            .saturating_sub(physical_y.max(0) as u32)
            .max(1);
        let rect = crate::dxgi_capture::MonitorRect {
            x: previous.physical_origin_x,
            y: previous.physical_origin_y,
            width: previous.physical_width,
            height: previous.physical_height,
        };
        let native = app
            .state::<AppState>()
            .dxgi_capture
        ...
        match native {
            Ok(frame) => {
                let mut rgba = frame.bytes;
                for pixel in rgba.chunks_exact_mut(4) {
                    pixel.swap(0, 2);
                }
                let full = RgbaImage::from_raw(frame.width, frame.height, rgba)
                    .ok_or_else(|| "DXGI frame dimensions are invalid".to_string())?;
                let crop_x = physical_x.max(0) as u32;
                let crop_y = physical_y.max(0) as u32;
                let crop_width = requested_width
                    .min(full.width().saturating_sub(crop_x))
                    .max(1);
                let crop_height = requested_height
                    .min(full.height().saturating_sub(crop_y))
                    .max(1);
                imageops::crop_imm(&full, crop_x, crop_y, crop_width, crop_height).to_image()
            }
            Err(error) => {
                eprintln!("{error}; falling back to compatibility scroll capture");
                screen
        ...
        if let Err(error) = worker_app.run_on_main_thread(move || {
            match result {
                Ok(capture) => {
                    ...
                    *URI_CAPTURE.lock().unwrap() = Some(capture.clone());
...
fn remember_confirmed_image(app: &AppHandle, data: &[u8]) {
    *app.state::<AppState>().confirmed_png.lock().unwrap() = Some(data.to_vec());
}
...
fn suspend_shortcut(app: &AppHandle) {
    ...
    let registered_shortcut = state.registered_shortcut.lock().unwrap().take();
...
fn register_shortcut(app: &AppHandle, shortcut: &str) -> Result<(), String> {
    ...
    let previous_shortcut = state.registered_shortcut.lock().unwrap().take();
...
#[tauri::command]
pub fn scroll_control_ready(state: State<AppState>) {
...
pub fn report_capture_rendered(app: AppHandle, generation: u64) {
        ...
        .find(|timing| timing.generation == generation)
    {
        ...
        eprintln!(
            "Screenshot timing generation {}: native={}ms, WebView={}ms",
            timing.generation,
            timing.native_ready_ms,
...
#[tauri::command]
pub fn finish_scroll_capture(app: AppHandle) -> bool {
    finish_scroll_impl(&app);
    true
}

#[tauri::command]
pub fn cancel_scroll_capture(app: AppHandle) -> bool {
    cancel_scroll_impl(&app);
    true
}
...
// DockMapper owns the application builder, tray and configuration lifecycle.
// Keep the upstream entry point for reference, but do not compile a second
// Tauri application (or its standalone LiteSnap settings/tray integration).
#[cfg(any())]
pub fn run() {
...
mod tests {
    ...
    fn pin_requires_a_confirmed_selection_png() {
        ...
        *state.confirmed_png.lock().unwrap() = Some(vec![1, 2, 3]);
        assert_eq!(
