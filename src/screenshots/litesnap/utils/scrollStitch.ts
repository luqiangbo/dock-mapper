...
/** Loads the current native screenshot without serialising it through Tauri IPC. */
export function loadImageFromUrl(url: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const image = new Image();
    // Must be assigned before src: the in-memory protocol has a distinct
    // Windows origin and is subsequently drawn to the annotation Canvas.
