...
| 按键展示效果 | `key-visualizer-effect-*` | 按键展示的每屏鼠标光圈、点击和定位动画；与按键窗口共享配置和输入服务，截图期间暂停。 |
...
截图场景以不可变冻结画面为底图。矩形、椭圆、菱形、线、箭头、画笔、高亮、马赛克、文字和编号统一进入带稳定 ID、seed、version、angle、group ID 与 z-order 的场景；预览、复制、保存、贴图与历史 PNG 共用同一按层渲染入口。截图历史每条记录仍只存储最终 PNG、manifest 与可重建缩略图，不保存可编辑 sidecar；默认清理超过 30 天或 100 条的未收藏记录。

## 关键实现边界

- **任务栏挂件**：自动合并非虚拟网卡流量；旧的指定网卡配置读取后会迁移为空。数值采用固定槽位，`ResizeObserver` 仅在配置变更后同步宽度，避免因实时数值变化导致任务栏抖动。
- **仪表盘**：维护至多 5 分钟的内存环形采样；主窗口隐藏时仍每 10 秒接收一次采样，并按分钟降采样后保存在本机，供 1 小时和 24 小时趋势使用；应用退出后的缺失数据以断点或空状态呈现，不伪造零值。阈值规则和挂件预设沿用原子 JSON 配置，提醒未读记录保存在主窗口本机存储。
- **按键文本**：使用 Raw Input 的 `RIDEV_INPUTSINK` 接收后台键盘事件，不使用低级 Hook。只转换键帽名称，绝不拦截按键、不保存原始设备信息或扫描码；启用后固定于主显示器工作区左下角。
- **截图标注**：有限画布场景 reducer 负责多选、单层分组、图层、复制、变换、绑定和 30 步手势级撤销；旋转命中使用逆变换几何。RoughJS 图形按不可变元素引用和完整样式缓存，元素 seed 在移动、缩放和撤销间稳定，导出强制重建。线和箭头支持节点编辑，箭头端点以目标局部归一化坐标绑定，目标变换时原子更新。
- **标注样式配置**：`annotation_styles` 按 shape、line、arrow、pen、highlight、text、number、mosaic 分工具保存；浮层使用乐观更新和串行原子写入，普通设置保存会保留最新样式。旧 `annotation_color`、`annotation_outline` 与兼容命令继续可读。
- **配置与 IPC**：设置使用原子 JSON 事务；普通 IPC 使用类型化绑定，图片等大二进制传输继续使用手写封装，不经 Base64 JSON。

按键、鼠标与锁定键效果统一由“按键展示”设置页管理，启用状态和偏好写入本机原子 JSON 配置。
