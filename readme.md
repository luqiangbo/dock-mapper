...
1. 配置导入/导出及应用前预览；导入映射不会自动写入注册表。
2. 快捷键占用状态与一键恢复默认。
3. 历史增加日期筛选，OCR 结果支持导出为纯文本或 Markdown；不增加全文搜索。
4. 任务栏挂件增加可选 CPU、磁盘和电池指标。

当前配置继续使用原子 JSON，截图历史继续使用 manifest、PNG 与派生缩略图。只有在加入 OCR 历史索引、标签、复杂筛选或历史规模达到数千条时才考虑由 Rust 后端引入 SQLite/FTS5；PNG 仍保存为文件，不向前端开放通用 SQL 查询。
